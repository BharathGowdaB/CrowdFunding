...
const Navbar = ({ setSearchAddress , searchAddress}) => {
  ...
  const {  address = "hi" } = useStateContext();
  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[32px] gap-6">
      ...
      <div className="lg:flex-1 md:flex-1 flex flex-row max-w-[432px] min-w-[200px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
        <input id={'searchAddress'} type="text" placeholder="Search for user" className="flex w-full font-epilogue font-normal text-[14px] placeholder:text-[#4b5264] text-white bg-transparent outline-none" />
        
        <div onClick={() => {
            let searchAddress = (document.getElementById('searchAddress').value || '').trim()
            if(!searchAddress) return;
            if(!searchAddress.startsWith('0x')) searchAddress = '0x' + searchAddress
          ...
          <img src={search} alt="get starter details" className="w-[15px] h-[15px] object-contain"/>
        </div>
      </div>
