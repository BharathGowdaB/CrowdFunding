...
const MilestoneCard = ({ milestoneAddress , projectAddress, isStarter ,userAddress, setIsLoading, setIsLogging, setLogger}) => {
  ...
  const { getMilestoneDetails, getMilestoneVotes, voteMilestone, processTransactionError, startMilestone, endMilestone } = useStateContext()
  ...
  const handleStartMilestone = async () => {
    ...
    try {
      ...
      setLogger({error: true, message, handleClick: () => {
        setIsLogging(false)
      } })
      setIsLogging(true)
    }
  ...
  const handleEndMilestone = async () => {
    setIsLoading(true)
    try {
      const tnx = await endMilestone(projectAddress, milestoneAddress, milestone.returnAmount)
      await tnx.wait()
  ...
  const handleVoting = async (vote) => {
      ...
      try {
        ...
        setLogger({error: false, message: 'Succefully Voted', handleClick: () => {
          setIsLogging(false)
        } })
        setIsRejected(vote)
      } catch(error) {
  ...
  return (
    <div className="w-full flex rounded-[8px] bg-[#1c1c24] cursor-pointer p-4 mb-[20px]" >
        <div className='flex-1 flex flex-col pr-4'>
          <div className="block flex-1">
            <div className='flex pr-[10px]'>
              <h3 className="flex-1 text-[24px] text-white text-left">{milestone.title}</h3>
            </div>
            
            <p className="flex-1 whitespace-wrap mt-[8px] font-epilogue font-normal text-[#808191] text-left whitespace-wrap">{milestone.description}</p>
          </div>
          <div className='text-[#b2b3bd] flex justify-between my-[20px]'>
            <div>
              <span className='text-white font-[500]'>{milestone.fundsRequired + ' ETH '}</span> required with expected return 
              <span className='text-[#8c6dfd] font-[500]'>{' '+ milestone.returnAmount + ' ETH '}</span>
            </div>
          </div>
          <div className='flex gap-4 font-[500] items-center text-[12px]  text-[#b2b3bd]'>
            <div className="flex-1">
              Address: {milestone.address}
            </div>
            <div className='flex w-fit items-center gap-2'>
              <div>Current Approved Percent: </div>
              <div className={`w-fit font-[700] text-[16px] text-[#1dc071] ${currentVote.votes / currentVote.maxVotes * 100.0 <  50 && 'text-[#ff0000]'}`}>
                  { `${Math.round(currentVote.votes / currentVote.maxVotes * 10000.0) / 100} %`}
                </div>
            </div>
        ...
        <div className='flex flex-col justify-center items-center w-[180px]'>
          {/* MilestoneState: initial, inVoting, inExecution, rejected, ended */}
                {milestone.state == 0 && 
                            <div className='flex flex-1 text-white font-[600] text-[16px] items-center'> 
                ...
                {milestone.state == 2 && 
                            <div className='flex flex-1 text-white font-[600] text-[16px] items-center'> 
                                ...
                                <div className='p-[10px] pl-[8px] min-h-[36px] text-[#b2b3bd]'>In Execution</div>
                            </div>}  
                {milestone.state == 4 && 
                            <div className='flex flex-1 text-white font-[600] text-[16px] items-center'> 
                                <img src={verified} title="Milestone Ended" alt="Milestone Ended" className="w-[36px] h-[36px] object-contain"/> 
                                <div className='p-[10px] pl-[8px] min-h-[36px] text-[#b2b3bd]'>Completed</div>
                            </div>}    
                {milestone.state == 3 &&
                            <div className='flex flex-1 text-white font-[600] text-[16px] items-center'> 
                                <img src={failed} title="Failed" alt="Failed" className="w-[36px] h-[36px] object-contain"/> 
                                <div className='p-[10px] pl-[8px] min-h-[36px] text-[#b2b3bd]'>Is Rejected</div>
                            </div>}
          {isStarter && <div>
            
          { milestone.state == 1 && <div>
              <CustomButton 
              type ="button"
          ...
          {  milestone.state == 2 && <div>
                <CustomButton 
                type ="button"
                title="End Milestone"
                styles='mt-[12px] w-fit min-w-[100px] font-[400] min-w-[170px] border-[1px] border-[#ff0000] rounded-[4px] p-[2px] text-[#ff0000]'
                handleClick={handleEndMilestone}
              />
              </div>}
          </div>}
          {!isStarter && milestone.state == 1 && !isRejected && <div>
            <CustomButton 
              type ="button"
